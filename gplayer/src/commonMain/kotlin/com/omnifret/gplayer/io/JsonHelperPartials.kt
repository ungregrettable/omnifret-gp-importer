...
import com.omnifret.gplayer.core.toIntOrNaN
import kotlin.contracts.ExperimentalContracts
import kotlin.reflect.KClass

internal open class JsonHelperPartials {
    companion object {
        // @JvmName disambiguates the JVM signature, which is otherwise
        // identical for both forEach overloads (lambda return type
        // erases). It's a no-op on Kotlin/Native, where the signatures
        // already differ structurally.
        @kotlin.jvm.JvmName("forEachBool")
        ...
        public fun <T : Enum<T>> parseEnum(value: String, values: Array<T>): T? {
            ...
            for (e in values) {
                if (valueLower.equals(e.name, true) ||
                    (!isNaN && e is com.omnifret.gplayer.core.IGPlayerEnum && e.value == valueInt.toInt())
                ) {
                    return e
                }
            }
        ...
        public fun <T : Enum<T>> parseEnum(value: Int, values: Array<T>): T? {
            for (e in values) {
                if (e is com.omnifret.gplayer.core.IGPlayerEnum && e.value == value) {
                    return e
                }
            }
            return null
        ...
        public inline fun <reified T : Enum<T>> parseEnum(
            ...
            return when (value) {
                ...
                else -> throw GPlayerError(
                    GPlayerErrorType.Format,

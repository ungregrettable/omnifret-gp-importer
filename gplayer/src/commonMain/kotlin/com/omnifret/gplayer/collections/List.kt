...
import com.omnifret.gplayer.core.toTemplate

// kotlin.collections.ArrayList is `final` on Kotlin/Native, so we can't
// extend it. Use interface delegation over a backing MutableList — the
// public surface is the same (List<T>'s _data field treats us as
// MutableList<T>), and removeRange is implemented via subList().clear()
// which is portable.
internal class ArrayListWithRemoveRange<T> private constructor(
    private val backing: MutableList<T>
) : MutableList<T> by backing {

    constructor() : this(mutableListOf())
    constructor(c: Collection<T>) : this(c.toMutableList())

    public fun removeRange(startIndex: Int, endIndex: Int) {
...
public class List<T> : Iterable<T> {
    ...
    internal constructor(items: com.omnifret.gplayer.collections.List<T>) {
